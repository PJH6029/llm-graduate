...
class TransformerManager(BasePipelineManager):       
    ...
    def transform(self, sentence: str, history: list[ChatLog]=None) -> dict[str, Union[str, list[str]]]:
        if self.transformer_name is None:
            msg.warn("Transformer not set. Skipping transformation.")
            return [sentence]

        history = history or []
        sentences: dict[str, Union[str, list[str]]] = {}
        ...
        return sentences # remove duplicates, preserving order
